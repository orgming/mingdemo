...
func TestContextHandlerName(t *testing.T) {
	...
	assert.Regexp(t, "^(.*/vendor/)?github.com/orgming/mingdemo/framework/gin.handlerNameTest$", c.HandlerName())
}
...
func TestContextHandlerNames(t *testing.T) {
	...
	for _, name := range names {
		assert.Regexp(t, `^(.*/vendor/)?(github\.com/orgming/mingdemo/framework/gin\.){1}(TestContextHandlerNames\.func.*){0,1}(handlerNameTest.*){0,1}`, name)
	}
}
