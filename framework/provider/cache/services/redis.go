...
import (
	...
	"github.com/orgming/ming/framework/provider/redis"
	"sync"
	"time"
)
...
func (r *RedisCache) SetForeverObj(ctx context.Context, key string, val any) error {
	return r.client.Set(ctx, key, val, NoneDuration).Err()
}
...
func (r *RedisCache) Remember(ctx context.Context, key string, timeout time.Duration, rememberFunc contract.RememberFunc, obj any) error {
