...
// MingGorm ming框架的orm服务实现
type MingGorm struct {
...
func NewMingGorm(params ...any) (any, error) {
...
// GetDB 获取DB实例
func (app *MingGorm) GetDB(option ...contract.DBOption) (*gorm.DB, error) {
	...
	if config.ConnMaxLifetime != "" {
		...
		if err != nil {
			logger.Error(context.Background(), "conn max lift time error", map[string]any{
				"err": err,
			})
		} else {
	...
	if config.ConnMaxIdletime != "" {
		...
		if err != nil {
			logger.Error(context.Background(), "conn max idle time error", map[string]any{
				"err": err,
			})
		} else {
