...
func GetBaseConfig(c framework.Container) *contract.SSHConfig {
	...
	if err != nil {
		...
		logService.Error(context.Background(), "parse cache config error", map[string]any{
			"err": err,
		})
		return nil
	}
...
func WithConfigPath(configPath string) contract.SSHOption {
	return func(container framework.Container, config *contract.SSHConfig) error {
		...
		if rsaKey, ok := conf["rsa_key"]; ok {
			...
			if err != nil {
				logService.Error(context.Background(), "read rsa_key error", map[string]any{
					"key":  rsaKey,
					"path": configPath,
					"err":  err,
				})
			...
			if err != nil {
				logService.Error(context.Background(), "create rsa_key signer error", map[string]any{
					"key":  rsaKey,
					"path": configPath,
					"err":  err,
				})
		...
		if knownHosts, ok := conf["known_host"]; ok {
			...
			if err != nil {
				logService.Error(context.Background(), "knownhosts error", map[string]any{
					"key":  knownHosts,
					"path": configPath,
					"err":  err,
				})
