...
func initAppCmd() *cobra.Command {
	appStartCmd.Flags().BoolVarP(&appDaemon, "daemon", "d", false, "start app daemon")
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if appDaemon {
			...
			cxt := &daemon.Context{
				...
				Umask: 027,
				// 子进程的参数，按照这个参数设置，子进程的命令为 ./ming app start --daemon=true
...
var appRestartCmd = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		appDaemon = true
		// 直接daemon方式启动apps
		return appStartCmd.RunE(c, args)
	},
}
