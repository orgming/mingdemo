...
import (
	...
	"github.com/orgming/ming/framework/provider/ssh"
...
var deployRollbackCmd = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		if len(args) != 2 {
			return errors.New("参数错误,请按照参数进行回滚 ./ming deploy rollback [version] [frontend/backend/all]")
		}
...
func deployBuildBackend(c *cobra.Command, deployFolder string) error {
	...
	if err != nil {
		log.Fatalln("ming go: 请在Path路径中先安装go")
	}
	...
	if err != nil {
		logger.Error(ctx, "go build err", map[string]any{
			"err": err,
			"out": string(out),
		})
		return err
...
func deployUploadAction(deployFolder string, container framework.Container, end string) error {
	...
	for _, node := range deployNodes {
		...
		for _, action := range preActions {
			...
			logger.Info(context.Background(), "execute pre action start", map[string]any{
				"cmd":        action,
				"connection": node,
			})
			// 执行命令，并且等待返回
			...
			session.Close()
			// 执行前置命令成功
			logger.Info(context.Background(), "execute pre action", map[string]any{
				"cmd":        action,
				"connection": node,
		...
		if err := uploadFolderToSFTP(container, deployFolder, remoteFolder, client); err != nil {
			logger.Info(context.Background(), "upload folder failed", map[string]any{
				"err": err,
			})
			return err
		}
		...
		for _, action := range postActions {
			...
			logger.Info(context.Background(), "execute post action start", map[string]any{
				"cmd":        action,
				"connection": node,
			})
			bts, err := session.CombinedOutput(action)
			...
			logger.Info(context.Background(), "execute post action finish", map[string]any{
				"cmd":        action,
				"connection": node,
				"out":        strings.ReplaceAll(string(bts), "\n", ""),
			})
		...
		if rfStat.Size() > 2*1024*1024 {
			...
			go func(localFile, remoteFile string) {
				...
				for range ticker.C {
					...
					if err != nil {
						logger.Error(context.Background(), "stat error", map[string]any{
							"err":         err,
...
func deployBuildFrontend(c *cobra.Command, deployFolder string) error {
	...
	// 编译前端
	if err := buildFrontendCmd.RunE(c, []string{}); err != nil {
		return err
	}
