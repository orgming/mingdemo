...
var providerListCmd = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		mingContainer := container.(*framework.MingContainer)
		list := mingContainer.NameList()
...
var providerCreateCmd = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		// 检查服务是否存在
		providers := container.(*framework.MingContainer).NameList()
		...
		// 创建title这个模版方法
		funcs := template.FuncMap{"title": strings.Title} // TODO
		{
			//  创建contract.go
